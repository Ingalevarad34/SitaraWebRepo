...
function Header() {
  ...
  const handleSearch = (e) => {
    ...
    const results = musicData.filter((song) => {
      ...
      return (
        musicName.includes(query.toLowerCase()) ||
        artistName.includes(query.toLowerCase())
      );
    });
  ...
  const addToPlaylist = async (song) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:8080/playListSongs/addPlayListSongs",
        [playlistSong]
      );
            ...
            {currentSong && (
              <div className="audio-player">
                ...
                <div className="player-info">
                  <strong>{currentSong.musicName}</strong> -{" "}
      ...
      <nav className="navbar navbar-expand-lg navbar-dark bg-transparent px-4">
        ...
        <div className="input-group input-design">
          <span className="input-group-text bg-color border-0">
            <i
              className="fa-solid fa-magnifying-glass fa-flip-horizontal"
              style={{ color: "#ffffff" }}
            ></i>
          </span>
