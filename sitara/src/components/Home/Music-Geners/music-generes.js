...
import '../Mood-Playlist/mood-playlist.css';
...
function MusicGenres() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/musicGenres-songs");
                    ...
                    <tbody>
                      {albumSongs.map((song, index) => (
                        <tr key={index} onClick={() => playSong(index)} style={{ cursor: "pointer" }}>
                          <td>{index + 1}</td>
                          <td><img src={song.imageUrl} style={{height:"45px",width:"90px"}} alt={song.musicName} /></td>
